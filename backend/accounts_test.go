...
func TestAccountsByKeystore(t *testing.T) {
	...
	ks2.RootFingerprintFunc = rootFingerprint2Func
	...
	require.Equal(t, 3, len(accountsMap[hex.EncodeToString(ks1Fingerprint)]))
	require.NotNil(t, accountsMap[hex.EncodeToString(ks2Fingerprint)])
	require.Equal(t, 3, len(accountsMap[hex.EncodeToString(ks2Fingerprint)]))
